 ...
 *  Changelog:
 *    1.2.2h (07/01/2021)
 *      - Migrated to Hubitat by Scott Miller (thanks Kevin!)
 *
 *    1.2.2 (12/12/2020)
...
def healthPoll() {
	logDebug "healthPoll()"
	sendHubCommand([new hubitat.device.HubAction(versionGetCmd())])
}
...
def zwaveEvent(hubitat.zwave.commands.basicv1.BasicSet cmd, endpoint=null) {
...
def soundSwitchEvent(hubitat.zwave.commands.multichannelv3.MultiChannelCmdEncap cmd) {
...
def zwaveEvent(hubitat.zwave.Command cmd, endpoint=null) {
